...
def require_plan(allowed_plans: list):
    ...
    def checker(user: User = Depends(get_current_user)):
        if not user.subscription_status or user.subscription_status not in allowed_plans:
            logger.warning(
                f"❌ Plan Access Denied for: {user.email} - Plan: {user.subscription_status}")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="⛔ Access denied: Please upgrade your plan to access this feature.")
        logger.info(
            f"✅ Plan Access Granted: {user.email} - Plan: {user.subscription_status}")
        return user
    return checker
...
def check_admin(current_user: User = Depends(get_current_user)):
    ...
    if current_user.role != "admin":
        logger.warning(
            f"🚫 Unauthorized Admin Attempt: {current_user.email} - Role: {current_user.role}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
