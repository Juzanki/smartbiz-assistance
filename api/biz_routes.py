...
def forward_injection(req: InjectionRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"❌ Injection forwarding failed: {str(e)}")
...
def forward_prompt(req: PromptRequest):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"❌ Prompt forwarding failed: {str(e)}")
...
def forward_execution():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"❌ Execution forwarding failed: {str(e)}")
...
def get_vision_report():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"❌ Vision report failed: {str(e)}")
...
def get_kernel_health():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"❌ Kernel health check failed: {str(e)}")
