...
# --- Create Smart Task ---
def create_task(db: Session, user_id: int, title: str, description: str, execute_at: datetime) -> SmartTask:
    task = SmartTask(
        user_id=user_id,
        title=title,
...
def get_pending_tasks(db: Session) -> List[SmartTask]:
    now = datetime.utcnow()
    return db.query(SmartTask).filter(SmartTask.executed == False, SmartTask.execute_at <= now).all()
