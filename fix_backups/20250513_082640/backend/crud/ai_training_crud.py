...
def create_training(db: Session, user_id: int, keyword: str, response: str) -> KeywordTraining:
...
def get_all_trainings(db: Session, user_id: int) -> List[KeywordTraining]:
    return db.query(KeywordTraining).filter(KeywordTraining.user_id == user_id).all()

# --- Get Response by Keyword ---
def get_response_by_keyword(db: Session, user_id: int, keyword: str) -> KeywordTraining:
