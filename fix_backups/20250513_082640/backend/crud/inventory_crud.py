...
from backend.models import Inventory, InventoryLog
...
def add_stock(db: Session, product_id: int, quantity: int) -> Inventory:
    ...
    # Log stock addition
    log = InventoryLog(product_id=product_id, quantity=quantity, action='add', timestamp=datetime.utcnow())
    db.add(log)
    db.commit()
...
def remove_stock(db: Session, product_id: int, quantity: int) -> Inventory:
    ...
    if inv and inv.quantity >= quantity:
        ...
        # Log stock removal
        log = InventoryLog(product_id=product_id, quantity=quantity, action='remove', timestamp=datetime.utcnow())
        db.add(log)
        db.commit()
