...
# --- Get Logs ---
def get_logs(db: Session, skip: int = 0, limit: int = 50) -> List[InjectionLog]:
    return db.query(InjectionLog).order_by(InjectionLog.timestamp.desc()).offset(skip).limit(limit).all()
