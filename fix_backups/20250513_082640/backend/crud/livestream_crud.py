...
def create_livestream(db: Session, user_id: int, title: str, is_public: bool, scheduled_at: datetime) -> Livestream:
    session = Livestream(
        user_id=user_id,
        title=title,
        is_public=is_public,
...
def get_upcoming_public_streams(db: Session) -> List[Livestream]:
    now = datetime.utcnow()
    return db.query(Livestream).filter(Livestream.is_public == True, Livestream.scheduled_at >= now).all()
