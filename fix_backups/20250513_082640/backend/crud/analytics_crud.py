...
from backend.models import MessageLog, SmartOrder, Payment

# --- Get Message Count ---
def count_messages(db: Session, user_id: int) -> int:
    return db.query(MessageLog).filter(MessageLog.sender_id == str(user_id)).count()

# --- Get Order Count ---
def count_orders(db: Session, user_id: int) -> int:
    return db.query(SmartOrder).filter(SmartOrder.customer_id == user_id).count()
